...
/**
 * Service for WUA signing with X.509 certificate support.
 * Loads certificate and private key via Spring Boot SSL Bundles.
 */
@Service
public class WpSigningService {

    private final List<Base64> x5cChain;
    private final ECKey ecJwk;

    public WpSigningService(SslBundles sslBundles) throws Exception {
        SslBundle bundle = sslBundles.getBundle("wp-signing");
        ...
        // Build x5c chain (just the WP cert for self-signed)
