package dev.kmandalas.authserver.util;
...
public final class JwtSignatureUtils {
    ...
    public static JWK extractKeyFromX5c(SignedJWT signedJWT) {
        try {
            ...
            logger.warn("Unsupported key type in x5c certificate: {}", certificate.getPublicKey().getAlgorithm());
            return null;
    ...
    public static boolean verifySignature(SignedJWT signedJWT, JWK jwk) {
        try {
            ...
            if (jwk.getKeyType() == KeyType.RSA) {
                ...
                logger.warn("Unsupported key type for signature verification: {}", jwk.getKeyType());
                return false;
            }
    ...
    public static boolean verifySignatureWithX5c(SignedJWT signedJWT) {
        ...
        if (key == null) {
            logger.warn("No x5c certificate chain in JWT header");
            return false;
        }
     ...
     * This correctly compares only the key material, ignoring metadata like kid, use, alg.
     *
     * @param key1 first JWK to compare
     * @param key2 second JWK to compare
     * @return true if the keys have the same thumbprint (same key material)
     */
    public static boolean keysMatch(JWK key1, JWK key2) {
        try {
            String thumbprint1 = key1.computeThumbprint().toString();
            String thumbprint2 = key2.computeThumbprint().toString();
            return thumbprint1.equals(thumbprint2);
        } catch (Exception e) {
